    ...
    public static void main(String[] args) throws InterruptedException {
       ...
       {"As", "2s", "3s", "4s", "5s", "6s", "7s", "8s", "9s", "Ts", "Js", "Qs", "Ks",
        ...
        while (true) {
            ...
            for (int x = 0; x < pairs.size() / 2; x++) {

                if (Objects.equals(pairs.get(pocket), pairs.get(pocket + 1))) {

                    List<Integer> pocketpairstrength = new ArrayList<>();
                    int counter2 = 0;
                        for (String t: handrank) {
                            counter2 ++;
                            if (t.equals(pairs.get(pocket))) {
                                pocketpairstrength.add(counter2);
                            }
                        }
                    for (String s : board) {
                        if (pairs.get(pocket).matches(String.valueOf(s.charAt(0)))) {
                            pocketpair++;
                        }
                    }
                     if (pocketpair == 2) {
                        pocketpair=0;
                        System.out.println("Quads " + pairs.get(pocket) + "s!");

                    } else if (pocketpair==0) {

                        if(boardpairs.size()==3) {
                            System.out.println("FullHouse! " + pairs.get(pocket) + boardpairs.get(0) + "s");
                        } else if (boardpairs.size()==2) {

                            if (pocketpairstrength.get(0) > test.get(0)) {
                                System.out.println("Two Pair! " + handrank[test.get(1)-1] + pairs.get(pocket) + "s");
                            ...
                            if (!Objects.equals(test.get(3), test.get(2))) {
                                ...
                                if (pocketpairstrength.get(0) < test.get(3)) {
                                    System.out.println("counterfeit pair: full house !" + pairs.get(pocket) + "s");
                                    pocketpairstrength.clear();
                                } else {
                                ...
                                if (pocketpairstrength.get(0) < test.get(0)) {
                                    ...
                                    System.out.println("Full House! " + handrank[test.get(3) -1] + pairs.get(0) + "s");
                                    pocketpairstrength.clear();
                                }
                            ...
                            System.out.println("Two Pair! " + boardpairs.get(0) + boardpairs.get(0)  + pairs.get(pocket) + pairs.get(pocket) + "s");
                            pocketpairstrength.clear();
                        }
                        else {
                            System.out.println("Pocket Pair " + pairs.get(pocket) + pairs.get(pocket + 1));
                            pocketpairstrength.clear();
                        }
                    }
                }
            ...
            for (String pair : pairs) {
                ...
                if (twopairarray.size() == 2 && innercount % 2 == 0) {
                    if (boardpairs.size() == 1) {
                        ...
                        if (Math.min(counterfeit.get(0), counterfeit.get(1)) > test.get(0)) {
                            System.out.println("Two Pair! " + twopairarray.get(0) + twopairarray.get(1) + "s");
                            counterfeit.clear();
                            twopairarray.clear();
                        } else if (test.get(0) > Math.max(counterfeit.get(0), counterfeit.get(1))) {
                            Collections.sort(counterfeit);
                            System.out.println("Two Pair! " + boardpairs.get(0) + boardpairs.get(0) + handrank[counterfeit.get(1) - 1] + handrank[counterfeit.get(1) - 1]);
                            counterfeit.clear();
                            twopairarray.clear();
                        }
                    } else {

                        twopairarray.add(twopairarray.get(0));
                        twopairarray.add(twopairarray.get(1));
                        System.out.println("Two Pair! " + twopairarray);
                        twopairarray.clear();
                    }
                }
                if (twopairarray.size() == 1 && innercount % 2 == 0) {
                    twopairarray.clear();
                }
            }
                int iteratecount = 0;
                ...
                for (String s : pairs) {
                    ...
                    if (hands.size()==4 && iteratecount%2==0 && Objects.equals(pairs.get(iteratecount-2), pairs.get(iteratecount-1))) {
                        if (boardpairs.size()==1 || boardpairs.size()==2) {
                            ...
                            System.out.println("FullHouse! " + hands.get(0) + boardpairs.get(0) + "s");
                            hands.clear();
                        } else {
                    ...
                    if (hands.size()==2 && iteratecount%2==0) {
                        if (boardpairs.size()==1) {
                            ...
                            if (test.get(0)<playerhandstrength.get(0)) {
